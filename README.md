...
- 直接回车：生成最近可用市场日的页面。
- 输入 `20260625` 这样的日期：生成指定日期页面；如果输入的是非交易日，程序会映射到上一交易日。

BAT 会按三步执行：

1. 更新本地缓存到本机日期对应的最新可用市场日。
2. 使用本地缓存生成报告和 dashboard。
3. 尝试发布到 GitHub Pages。
...
这个指标适合做资金流监控和研究观察。正式生产使用前，还需要继续核对 ETF 总份额口径、份额折算、基金转型、清盘等生命周期事件。
